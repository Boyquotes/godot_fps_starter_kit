...
func process_UI(delta):
	...
	if current_weapon_name == "UNARMED" or current_weapon_name == "KNIFE":
		## First line: Health, second line: Armor Third line: Grenades
#		UI_status_label.text = "HEALTH: " + str(health) + \
		UI_status_label.text = "HP: " + str(health)+ "/" + str(MAX_HEALTH) + \
		"\nAP: " + str(armor) + "/" + str(MAX_ARMOR) + \
		"\n" + \
...
func add_max_health(additional_health):
	## Adds addition_health to our player, and clamps the health from 0 to MAX_HEALTH
	MAX_HEALTH += additional_health
	health = clamp(health, 0, MAX_HEALTH)


func add_armor(additional_armor):
	## Adds addition_armor to our player, and clamps the armor from 0 to MAX_ARMOR
	armor += additional_armor
	armor = clamp(armor, 0, MAX_ARMOR)
	
	
func add_max_armor(additional_armor):
	## Adds addition_armor to our player, and clamps the armor from 0 to MAX_ARMOR
	MAX_ARMOR += additional_armor
	armor = clamp(armor, 0, MAX_ARMOR)


func add_gold(additional_gold):
	## Adds addition_gold to our player, and clamps the health from 0 to MAX_GOLD
	playergold += additional_gold
...
func add_clip_size_pistol(additional_ammo):
	if (current_weapon_name != "UNARMED"):
		if (weapons["PISTOL"].CAN_REFILL == true):
			weapons["PISTOL"].AMMO_IN_MAG = weapons["PISTOL"].AMMO_IN_MAG + additional_ammo


func add_clip_size_rifle(additional_ammo):
	if (current_weapon_name != "UNARMED"):
		if (weapons["RIFLE"].CAN_REFILL == true):
			weapons["RIFLE"].AMMO_IN_MAG = weapons["RIFLE"].AMMO_IN_MAG + additional_ammo


func add_clip_size_shotgun(additional_ammo):
	if (current_weapon_name != "UNARMED"):
		if (weapons["SHOTGUN"].CAN_REFILL == true):
			weapons["SHOTGUN"].AMMO_IN_MAG = weapons["SHOTGUN"].AMMO_IN_MAG + additional_ammo


func add_clip_size_chaingun(additional_ammo):
	if (current_weapon_name != "UNARMED"):
		if (weapons["CHAINGUN"].CAN_REFILL == true):
			weapons["CHAINGUN"].AMMO_IN_MAG = weapons["CHAINGUN"].AMMO_IN_MAG + additional_ammo


func add_max_pistol_ammo(additional_ammo):
	if (weapons["PISTOL"].CAN_REFILL == true):
		weapons["PISTOL"].spare_ammo = weapons["PISTOL"].spare_ammo + additional_ammo


func add_max_rifle_ammo(additional_ammo):
	if (weapons["RIFLE"].CAN_REFILL == true):
		weapons["RIFLE"].spare_ammo = weapons["RIFLE"].spare_ammo + additional_ammo


func add_max_shotgun_ammo(additional_ammo):
	if (weapons["SHOTGUN"].CAN_REFILL == true):
		weapons["SHOTGUN"].spare_ammo = weapons["SHOTGUN"].spare_ammo + additional_ammo


func add_max_chaingun_ammo(additional_ammo):
	if (weapons["CHAINGUN"].CAN_REFILL == true):
		weapons["CHAINGUN"].spare_ammo = weapons["CHAINGUN"].spare_ammo + additional_ammo
...
func add_grenade(additional_grenade):
	...
	grenade_amounts["Sticky Grenade"] += additional_grenade
	grenade_amounts["Grenade"] = clamp(grenade_amounts["Grenade"], 0, MAX_NADES)
	grenade_amounts["Sticky Grenade"] = clamp(grenade_amounts["Sticky Grenade"], 0, MAX_STICKY)


func add_sticky(additional_grenade):
	## Adds additional_grenade to our player's current selected grenade, and clamps the amount from 0 to 4.
	grenade_amounts["Sticky Grenade"] += additional_grenade
	grenade_amounts["Sticky Grenade"] = clamp(grenade_amounts["Sticky Grenade"], 0, MAX_STICKY)


func add_phy_damage(additional_phydamage):
	## Adds additional_grenade to our player's current selected grenade, and clamps the amount from 0 to 4.
	P_DAMAGE += additional_phydamage


func add_exp_damage(additional_damage):
	## Adds additional_grenade to our player's current selected grenade, and clamps the amount from 0 to 4.
	damage += additional_damage

	
func add_max_groundspeed(additional_speed):
	MOVE_SPEED += additional_speed
	
	
func add_max_jump(additional_jump):
	JUMP_SPEED += additional_jump


func add_max_nade(additional_grenade):
	MAX_NADES += additional_grenade


func add_max_sticky(additional_grenade):
	MAX_STICKY += additional_grenade
...
func save(save_game : Resource):
	save_game.data[SAVE_KEY] = {
		...
		'SPRINT_ACCEL' : SPRINT_ACCEL,
		'GRAVITY' : GRAVITY,
		'damage' : damage,
		'P_DAMAGE' : P_DAMAGE,
		'playergold' : playergold,
		'MAX_GOLD' : MAX_GOLD,
		'grenade_amounts' : grenade_amounts,
		'has_key1' : has_key1,
		'has_key2' : has_key2,
		'has_key3' : has_key3,
		'has_rifle' : has_rifle,
		'has_shotgun' : has_shotgun,
		'has_chaingun' : has_chaingun,
		'big_talon_amount' : big_talon_amount,
		'big_bone_amount' : big_bone_amount,
		'big_tooth_amount' : big_tooth_amount,
		'big_claw_amount' : big_claw_amount,
		'level' : level,
			}
...
func load(save_game : Resource):
	...
	MAX_SPRINT_SPEED = data['MAX_SPRINT_SPEED']
	SPRINT_ACCEL = data['SPRINT_ACCEL']
	...
	grenade_amounts = data['grenade_amounts']
	has_key1 = data['has_key1']
	has_key2 = data['has_key2']
	has_key3 = data['has_key3']
	has_rifle = data['has_rifle']
	has_shotgun = data['has_shotgun']
	has_chaingun = data['has_chaingun']
	big_talon_amount = data['big_talon_amount']
	big_bone_amount = data['big_bone_amount']
	big_tooth_amount = data['big_tooth_amount']
	big_claw_amount = data['big_claw_amount']
	level = data['level']


func change_save_var(e):
	if e == 0:
		loadstate = 0
	if e == 1:
		loadstate = 1
...
func remove_big_talon():
	big_talon_amount -= 1
func remove_big_bone():
	big_bone_amount -= 1
func remove_big_claw():
	big_claw_amount -= 1
func remove_big_tooth():
	big_tooth_amount -= 1
