...
onready var obj = get_parent().get_node("radament")

var radament_node = null
		...
		if current_target != null:
			...
			if enemy_health > 0:
				...
				if ammo_in_turret > 0:
					...
					## If not, then refill the enemie's ammo.
...
func fire_bullet():
	...
	if use_raycast == false:
		...
		## Remove the bullet from the enemy
		ammo_in_turret -= 1
	...
	## If the enemy is out of ammo, set the reload timer
	if ammo_in_turret <= 0:
