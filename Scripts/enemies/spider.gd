...
onready var obj = get_parent().get_node("smallspider")

var smallspider_node = null
...
func _physics_process(delta):
	...
	if is_dying == true:
		...
		if soundvar == 3:
			create_sound("sp_death4")
			soundvar = 0
		elif soundvar == 2:
			create_sound("sp_death3")
			soundvar = 3
		elif soundvar == 1:
			create_sound("sp_death2")
			soundvar = 2
		elif soundvar == 0:
			create_sound("sp_death1")
			soundvar = 1
		is_dead = true
	...
	## If one of the bodies do, then call it and pass the area's global origin as the bullet collision point.
		...
		## If it is, then do nothing.
			...
			if body.has_method("P_hit"):
				...
				hitstun = 100
				if soundvar == 3:
					create_sound("sp_attack4")
					soundvar = 0 
				elif soundvar == 2:
					create_sound("sp_attack3")
					soundvar = 3
				elif soundvar == 1:
					create_sound("sp_attack2")
					soundvar = 2
				elif soundvar == 0:
					create_sound("sp_attack1")
					soundvar = 1
	...
	if body.has_method("process_input"):
		if is_dying == false:
			if is_dead == false:
				...
				current_target = null
				if soundvar == 3:
					create_sound("sp_idle4")
					soundvar = 0
				elif soundvar == 2:
					create_sound("sp_idle3")
					soundvar = 3
				elif soundvar == 1:
					create_sound("sp_idle2")
					soundvar = 2
				elif soundvar == 0:
					create_sound("sp_idle1")
					soundvar = 1
...
func bullet_hit(damage, bullet_hit_pos):
	...
	anim.play("gethit")
	if soundvar == 3:
		create_sound("sp_gethit4")
		soundvar = 0
	elif soundvar == 2:
		create_sound("sp_gethit3")
		soundvar = 3
	elif soundvar == 1:
		create_sound("sp_gethit2")
		soundvar = 2
	elif soundvar == 0:
		create_sound("sp_gethit1")
		soundvar = 1
