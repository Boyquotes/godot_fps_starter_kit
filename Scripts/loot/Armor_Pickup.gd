...
func _ready():
	...
	$Holder/Armor_Kit_Pickup_Trigger.connect("body_entered", self, "trigger_body_entered")
...
func kit_size_change_values(size, enable):
	...
	if size == 0:
		$Holder/Armor_Kit_Pickup_Trigger/Shape_Kit.disabled = !enable
		$Holder/Armor_Kit.visible = enable
	elif size == 1:
		$Holder/Armor_Kit_Pickup_Trigger/Shape_Kit_Small.disabled = !enable
		$Holder/Armor_Kit_Small.visible = enable
...
func trigger_body_entered(body):
	...
	if body.has_method("add_armor"):
		body.add_armor(ARMOR_AMOUNTS[kit_size])
